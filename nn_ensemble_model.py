...
from nn_ensemble_utils import build_nn_ensemble

class nn_ensemble_model(base_model):
    def __init__(self, num_layers, hids, dims, context_dims, 
            ...
            multihead=False, fixed_masks=True, ensemble_size=10):
        self.dims = dims
        self.model = build_nn_ensemble(hids=hids, dims=dims, 
            context_dims=context_dims, device = device, multihead=multihead,
            fixed_masks =fixed_masks, ensemble_size=ensemble_size).to(device)
    ...
    def sample_and_log_prob(self, numb_samps, context, kwargs={},
        ...
        output_hat, log_prob, mask_prob, mu, sig = (
            self.model.sample_and_log_prob(numb_samps, context = context,
                    kwargs=kwargs))
        return output_hat, log_prob, mask_prob, mu, sig
